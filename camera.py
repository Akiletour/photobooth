...
def TakePictures():
        ...
        thumb1 = image1.resize((530, 611))
        image2 = PIL.Image.open(filename2)
        thumb2 = image2.resize((530, 611))
        image3 = PIL.Image.open(filename3) 
        thumb3 = image3.resize((530, 611))
        image4 = PIL.Image.open(filename4) 
        thumb4 = image4.resize((530, 611))
        TotalImageCount = TotalImageCount + 1
	
        bgimage.paste(thumb1, (20, 20))
        bgimage.paste(thumb2, (560, 20))
        bgimage.paste(thumb3, (20, 655))
        bgimage.paste(thumb4, (560, 655))
        # Create the final filename
        ts = time.time()
        Final_Image_Name = os.path.join(imagefolder, "Final_" + str(TotalImageCount)+"_"+str(ts) + ".jpg")
        # Save it to the usb drive
        bgimagergba = bgimage.convert('RGB')
        bgimagergba.save(Final_Image_Name)
        # Save a temp file, its faster to print from the pi than usb
        bgimagergba.save('/home/pi/Desktop/tempprint.jpg')
        ShowPicture('/home/pi/Desktop/tempprint.jpg',3)
        #bgimage2 = bgimagergba.rotate(90)
        #bgimage2.save('/home/pi/Desktop/tempprint.jpg')
        ImageShowed = False
        Message = "Appuyez pour imprimer"
        UpdateDisplay()
        time.sleep(3)
        Message = ""
        UpdateDisplay()
        ...
        if Printing:
                if (TotalImageCount <= PhotosPerCart):
                        if os.path.isfile('/home/pi/Desktop/tempprint.jpg'):
                                ...
                                # select printer 0
                                selphy = printers.keys().index('Canon_SELPHY_CP1200');
                                printer_name = printers.keys()[selphy]
                                ...
                                if printqueuelength > 1:
                                        ...
                                        conn.printFile(printer_name, '/home/pi/Desktop/tempprint.jpg', "PhotoBooth", {"orientation-requested": "3"})
                                        time.sleep(55)            
                else:
